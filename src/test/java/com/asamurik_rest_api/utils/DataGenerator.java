    ...
    public String dataUsername() {
        ...
        while(!isValid){
            try{
                ...
                matcher = Pattern.compile("^[a-z0-9.]{3,50}$").matcher(usrName);
                isValid = matcher.find();
                if(intLoop==250){
    ...
    public String dataNamaLengkap() {
        ...
        while(!isValid){
            try{
                ...
                matcher = Pattern.compile("^[a-zA-Z\\s]{4,100}$").matcher(namaLengkap);
                isValid = matcher.find();
                if(intLoop==250){
...
//                matcher = Pattern.compile("^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[_#\\-$])(?!.*?[^A-Za-z0-9_#\\-$]).{8,}$").matcher(password);
                matcher = Pattern.compile("^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[@_#\\-$]).{8,}$").matcher(password);
                isValid = matcher.find();
